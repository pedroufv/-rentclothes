<div class="row">
    <div class="col-md-12">
        <a href="{{ route('address_user.create') }}" class="btn btn-success">
        ...
        <table class="table table-bordered">
            ...
            @foreach($user->addresses as $address)
                <tr>
                    ...
                    <td>{{ $address->city }}</td>
                    <td>
                        <a href="{{ route('address_user.show',['id' => $address->id]) }}">
                            <span class='glyphicon glyphicon-search'></span>
                        </a> |
                        <a href="{{  route('address_user.edit',['id' => $address->id]) }}">
                            <span class='glyphicon glyphicon-pencil'></span>
                        </a> |
                        <form onclick="return destroy(this, event)"
                              method="POST"
                              id='address_user-destroy'
                              style="display: inline; color: #3097d1;"
                              class="form-horizontal"
                              role="form"
                              action="{{ route('address_user.destroy', ['id' => $address->id]) }}">
